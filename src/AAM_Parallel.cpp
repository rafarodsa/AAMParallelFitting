...
#pragma omp parallel
	{
    #pragma omp for schedule(dynamic,BIGCHUNK) reduction(+:mean)
    ...
    #pragma omp for schedule(dynamic,BIGCHUNK) reduction(+: norm)
		for (int i = 0; i < np; i++) {
     ...
     #pragma omp single
		...
		#pragma omp for schedule(dynamic,BIGCHUNK) reduction(+: alpha)
		for (int i = 0; i < np; i++) {
			for (int k = 0; k < model_channels; k++) {
	...
	if (alpha != 0) {
		#pragma omp parallel for schedule(dynamic,BIGCHUNK)
		for (int i = 0; i < np; i++) {
			for (int k = 0; k < model_channels; k++) {
...
double AAM_Parallel::ComputeEstimationError(IplImage* image, double* __uc, double* __uq) {
  ...
  #pragma omp parallel
	{
    #pragma omp for schedule(dynamic,BIGCHUNK) reduction(+:error)
...
void AAM_Parallel::ParamsUpdate(IplImage* image) {
	...
	for (k = 0; k < __model->nModes() + 4; k++) {
		val = 0.0;
    #pragma omp parallel for schedule(dynamic,BIGCHUNK) reduction(+:val)
		for (int i = 0; i < __model->__texture.nPixels(); i++) {
...
void AAM_Parallel::ComputeNewParams(double k, double* __uc, double* __uq) {
	...
	#pragma omp parallel
	{
		#pragma for schedule(dynamic,CHUNK) nowait private(i)
