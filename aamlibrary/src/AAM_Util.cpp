	...
	for(int y = miny; y < maxy; y++)
	{
		...
		for(int x = minx; x < maxx; x++)
		{
			...
			if(idx1 >= 0)
			{
				...
				// cout << "computed coordinates..." <<endl;
				// cout << " X: " << x << " Y :" << y << endl;
				// cout << " x2: " << x2 << " y2: " << y2 << endl;
				// cout << "Rect2 sizeY: " << rect2.size() << " sizeX: " << rect2[0].size() << endl;
				if(y2 < 0 || x2 < 0 || y2 >= rect2.size() || x2 >= rect2[0].size())  continue;
				idx2 = rect2[y2][x2];
